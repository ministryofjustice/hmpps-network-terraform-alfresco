...
resource "aws_security_group" "alfresco_external_lb_in" {
  ...
  tags        = "${merge(data.terraform_remote_state.vpc.tags, map("Name", "${var.environment_name}_${var.alfresco_app_name}_external-lb_in_in", "Type", "WEB"))}"
...
resource "aws_security_group" "alfresco_nginx_in" {
  ...
  tags        = "${merge(data.terraform_remote_state.vpc.tags, map("Name", "${var.environment_name}_${var.alfresco_app_name}_nginx_in", "Type", "WEB"))}"
...
resource "aws_security_group" "alfresco_db_in" {
  ...
  tags        = "${merge(data.terraform_remote_state.vpc.tags, map("Name", "${var.environment_name}_${var.alfresco_app_name}_db_in", "Type", "DB"))}"
...
resource "aws_security_group" "alfresco_internal_lb_in" {
  ...
  tags        = "${merge(data.terraform_remote_state.vpc.tags, map("Name", "${var.environment_name}_${var.alfresco_app_name}_internal-lb_in_in", "Type", "WEB"))}"
...
resource "aws_security_group" "alfresco_api_in" {
  ...
  tags        = "${merge(data.terraform_remote_state.vpc.tags, map("Name", "${var.environment_name}_${var.alfresco_app_name}_api_in", "Type", "API"))}"
...
resource "aws_security_group" "alfresco_elasticache_in" {
  name        = "${var.environment_name}-delius-core-elasticache-in"
  vpc_id      = "${data.terraform_remote_state.vpc.vpc_id}"
  description = "elasticache incoming"
  tags        = "${merge(data.terraform_remote_state.vpc.tags, map("Name", "${var.environment_name}_elasticache_in", "Type", "DB"))}"
